...
| `install-on-vps.sh`                   | The bash that the workflow's "Install + recompose" step runs inside the openstatus WSL2 distro: copies staging artifacts into `/opt/openstatus`, then `docker compose pull && up -d`.                |
...
one required reviewer per `docs/reference/SECRETS-AND-VARIABLES.md`).
