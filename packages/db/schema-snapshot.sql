...
CREATE TABLE public.dppo_annual_adjustment (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT dppo_annual_adjustment_advances_paid_provenance_chk CHECK ((num_nulls(advances_paid_amount, advances_paid_source, advances_paid_reference, advances_paid_recorded_at) = ANY (ARRAY[0, 4]))),
    CONSTRAINT dppo_annual_adjustment_exclude_loss_activity_provenance_chk CHECK ((num_nulls(exclude_loss_making_main_activity_amount, exclude_loss_making_main_activity_source, exclude_loss_making_main_activity_reference, exclude_loss_making_main_activity_recorded_at) = ANY (ARRAY[0, 4]))),
    CONSTRAINT dppo_annual_adjustment_exempt_revenue_provenance_chk CHECK ((num_nulls(exempt_revenue_amount, exempt_revenue_source, exempt_revenue_reference, exempt_revenue_recorded_at) = ANY (ARRAY[0, 4]))),
    CONSTRAINT dppo_annual_adjustment_loss_carry_forward_provenance_chk CHECK ((num_nulls(loss_carry_forward_amount, loss_carry_forward_source, loss_carry_forward_reference, loss_carry_forward_recorded_at) = ANY (ARRAY[0, 4]))),
    CONSTRAINT dppo_annual_adjustment_non_deductible_expenses_provenance_chk CHECK ((num_nulls(non_deductible_expenses_amount, non_deductible_expenses_source, non_deductible_expenses_reference, non_deductible_expenses_recorded_at) = ANY (ARRAY[0, 4]))),
    CONSTRAINT dppo_annual_adjustment_tax_reliefs_provenance_chk CHECK ((num_nulls(tax_reliefs_amount, tax_reliefs_source, tax_reliefs_reference, tax_reliefs_recorded_at) = ANY (ARRAY[0, 4]))),
