...
export function buildPostingKickoff(plan: BrainPostingPlan): string {
  ...
  return [
    ...
    `   mcp__afframe__list_accounts with periodId="${posting.periodId}" AND number set to that exact account`,
    "   number — it returns just that one account; read its id. Do NOT call list_accounts without a number: a",
    "   full period chart is large and overflows the tool-output limit. Prefer the 3-digit synthetic account.",
