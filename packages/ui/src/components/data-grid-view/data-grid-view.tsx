...
import { useCellFocusGrid, useScrollEdges } from "./data-grid-view-hooks"

/**
 * Split items into their left-pinned / unpinned-centre / right-pinned groups
 * (TanStack already returns headers / cells / leaf columns in that visual order).
 * The header row, every body row, and the summary row all render as left · centre
 * · flex-1 spacer · right, so the three-way split lives here instead of being
 * hand-rolled per rowgroup. The spacer stays at each call site — it differs (the
 * body spacer is transparent; the header + summary spacers carry `bg-grid-header`).
 */
function partitionByPin<T>(
  items: readonly T[],
  getPinned: (item: T) => "left" | "right" | false,
  ...
  for (const item of items) {
    ...
    if (pinned === "left") left.push(item)
    else if (pinned === "right") right.push(item)
...
export function DataGridView<TData>({
  ...
  // Two extracted subsystems (see data-grid-view-hooks): the pinned-column edge
  // shadows track the horizontal scroll position + column footprint; the cell
  // focus grid owns the arrow/Home/End/Page navigation, Tab-in auto-select, and
  // click-away clear.
  const { edges, updateEdges } = useScrollEdges(gridRef, [
    totalWidth,
    colCount,
    rowCount,
  ])
  ...
  const renderRow = (
    ...
    const { left, center, right } = partitionByPin(cells, (c) =>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {table.getHeaderGroups().map((headerGroup, rowIndex, groups) => {
            const { left, center, right } = partitionByPin(
              headerGroup.headers,
      ...
      {summaryRow ? (
        ...
        >
          {(() => {
            const summary = partitionByPin(leafColumns, (c) => c.getIsPinned())
            const cell = (column: (typeof leafColumns)[number]) => (
            ...
            return (
              <div
                ...
                className="flex w-full"
              >
                {summary.left.map(cell)}
                {summary.center.map(cell)}
                <div
                  data-slot="grid-summary-spacer"
                  className="flex-1 bg-grid-header"
                />
                {summary.right.map(cell)}
              </div>
            )
          })()}
        </div>
