...
spine through the shared `brain-gate` skill (`/brain-gate` in Claude Code, `$brain-gate` in Codex) and
verify it green BEFORE Hleb reviews. `.claude/workflows/brain-gate.js` remains the canonical two-reviewer
runner; the skill defines the equivalent Codex path when `Workflow` is unavailable. The companion
`.claude/workflows/thermo-review.js` is the Fable-5-high code-quality gate.
