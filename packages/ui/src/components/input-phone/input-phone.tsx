...
interface PhoneInputCountryProps extends Omit<
...
function PhoneInputCountry(props: PhoneInputCountryProps) {
  const {
    ...
    <Popover open={open} onOpenChange={onOpenChange} {...popoverProps}>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={isDisabled}
          className={cn(
            "h-full shrink-0 gap-1.5 rounded-r-none border-r border-input pr-1.5 pl-2.5 text-muted-foreground hover:bg-muted/60 focus-visible:z-10",
            className,
          )}
        >
          {!countryContext ? (
...
function PhoneInputField(props: React.ComponentProps<typeof Input>) {
  const {
...
export {
  ...
  type Country as PhoneInputCountryData,
