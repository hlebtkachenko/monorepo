...
Only `--category` (one of Added/Changed/Deprecated/Removed/Fixed/Security/Docs/Dependencies) and `--entry` are required. Optional frontmatter: `--bump` (patch|minor|major → the suggested version bump *level*, never a concrete `vX.Y.Z`), `--override` (marks the bump as deliberate — the release agent honors it without arguing against a rule-derived level), `--breaking` and `--migration` (each renders a hoisted callout atop the version section). Do not delete another PR's fragment — fragments are consumed only at release-cut.

**Author one fragment per meaningful change, not one per PR.** The gate's floor is one, but a PR that does three distinct user-facing things writes three `changelog:add` calls — many fragments per PR is expected and keeps `CHANGELOG.md` rich after squash-merge. Never scrape raw commit messages (that reintroduces `wip` / `fix review` noise); the fragment body is the curated, human-readable bullet.
