...
- UI: Form section layout pass — container-query responsive columns (stack the title above the fields on a narrow panel instead of cramming), `px-6`/`py-4` padding (24px sides = 3× the panel header), h3 (`Heading level={3}`) title; new `sectionSpace` gap section + a section `fill` flag so Empty fills while Form/Space take natural height and the body scrolls
