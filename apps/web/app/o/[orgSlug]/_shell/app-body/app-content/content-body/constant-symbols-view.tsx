...
/**
 * ConstantSymbolsView — Finance → Číselníky → Konstantní symboly.
 *
 * A READ-ONLY Table archetype over the shared `constant_symbol` register. No
 * inline edit / create / destructive action (reference data): search + column
 * display + a per-row read-only Inspector, plus the design-system selection
 * footer. Rows + localized names are assembled server-side; this view renders
 * them.
 */
export function ConstantSymbolsView({
  slug,
  title,
  rows,
  favorite,
  ...
  const t = useTranslations("org.constantSymbols")
  ...
  const selectionActions = React.useCallback(
    (
      ...
      return buildTableFooter(table, {
        exportFileName: "konstantni-symboly",
        selectedIds: ids,
        onCopyLink: (linkIds) => {
