...
- Required checks (14 contexts in `.github/rulesets/main.json`): `ci`, `gitleaks`, `lint` (commitlint), `Analyze (javascript-typescript)` (CodeQL), `review` (dependency-review), `scan-pr / osv-scan` (osv-scanner), `knip`, `check` (paired-files), `boundaries`, `conv-title`, `size-cap`, `shellcheck`, `cdk-synth-strict (staging)`, `cdk-synth-strict (production)`. A failure in any of these blocks merge. The full required/advisory matrix lives in `docs/conventions/CI-POLICY.md`, treat that file as the single source of truth.
