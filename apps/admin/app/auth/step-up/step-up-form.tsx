...
interface Props {
  /** Server-derived: the operator has 2FA enrolled → collect + require a code. */
  twoFactorEnrolled: boolean
  next: string
  email: string
}

export function StepUpForm({ twoFactorEnrolled, next, email }: Props) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true)
    try {
      const r = await verifyStepUpAction({ password, code, next })
      if (!r.ok) {
    ...
    <div className="flex flex-col gap-8">
      <header className="flex flex-col gap-2">
        ...
        <Text variant="muted">
          ...
          {twoFactorEnrolled ? " and your two-factor code" : ""} to continue.
        </Text>
      </header>
