...
- **closing**: time-versioned `organization_tax_profile` table (has_employees) wired into the statutory obligation engine so payroll obligations become real, plus a Settings > Tax profile capture page and a real Payroll landing (period-mechanism PR3d).
