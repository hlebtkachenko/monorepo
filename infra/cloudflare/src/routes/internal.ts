  ...
  async (c) => {
    ...
    await deleteOldCache({
      ...
      BUCKET_OBJECT_EXPIRATION_HOURS:
        expireInHours ?? c.env.BUCKET_OBJECT_EXPIRATION_HOURS,
    })
...
internalRouter.post(
  ...
  vValidator(
    "json",
    v.object({ count: v.pipe(v.number(), v.maxValue(1000), v.minValue(1)) }),
  ),
  async (c) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      promises.push(
        storage.write(key, emojis[Math.floor(Math.random() * emojis.length)]),
      )
    }
