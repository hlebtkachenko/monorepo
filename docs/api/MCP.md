...
Reference design for the official Model Context Protocol server exposing `api.afframe.com/v1` to LLM clients (Claude Desktop, Claude Code, Cursor, ChatGPT desktop, ...).
...
| Schema source          | Auto-generate from OpenAPI 3.1, hand-curate, commit as source (don't regenerate at runtime) | Speakeasy / `cnoe-io/openapi-mcp-codegen` for the codegen pass. Repair pass gets ~94% usable.                         |
