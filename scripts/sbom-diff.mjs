...
// JSON results file. Exits non-zero on (for any component ADDED or upgraded to
// a new version relative to the previous SBOM — DEV-79):
//   (a) the component carrying a GPL/AGPL/LGPL license, or
//   (b) the component having a HIGH or CRITICAL CVE in the osv-scanner
//       results.
//
// Usage:
...
if (added.length > 0) {
  ...
  for (const c of added) {
    stdout.write(
      `  + ${c.name}@${c.version} [${licenseStrings(c).join(", ") || "no-license"}]\n`,
    )
  }
}
if (removed.length > 0) {
...
// Gate added components AND the NEW version of any upgraded component: a
// version bump can introduce a copyleft license or a HIGH/CRITICAL CVE even
// though the component name already existed in the previous SBOM (DEV-79).
const gated = [...added, ...versionChanges.map((v) => v.component)]
for (const c of gated) {
  ...
  if (hit) {
    cveHits.push(
      `${c.name}@${c.version} :: ${hit.severity} (${(hit.ids ?? []).join(", ")})`,
    )
  }
}
