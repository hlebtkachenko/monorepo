...
> legacy AFF-247) — see the explicit
...
| Art. 11 | Response & recovery           | **Rotation drill: DONE** (M10, 2026-05-31, end-to-end verified). **DR restore drill: NOT YET RUN** — deferred to legacy AFF-247; restore RTO is therefore UNVERIFIED. Backup itself is live (restic→R2, 6h). |
...
- **Single-node Vault** — no HA cluster today; mitigated by 6-hour restic backups to R2. **Restore RTO is a TARGET (≤30 min), not yet measured** — the DR drill that would verify it is deferred (legacy AFF-247).
- **Single-operator escrow** — 5 recovery keys (3-of-5 quorum) controlled by one person, held in **offline dual-custody escrow (off-repo)** (proven working 2026-05-31: 3 keys regenerated root during the M3.5 cascade recovery). Daily admin is a 90-day Keychain operator-admin token. Adding a second escrow operator is tracked in legacy AFF-245's post-100-clients section.
- **File-based audit device** — local log on VPS. Mitigated by 13-month retention deferral until SOC 2 (legacy AFF-244).
...
- Dynamic DB secrets (legacy AFF-243)
