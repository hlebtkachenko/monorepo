...
> start. **New here? Start at [`SECRETS-ADD-DELETE.md`](../runbooks/SECRETS-ADD-DELETE.md)**
...
| `AWS_DEPLOY_ROLE_ARN_BETA`, `CLOUDFLARE_TUNNEL_TOKEN_BETA`        | environment `secrets`                | `beta`                                                            | beta portal's own least-privilege deploy role + its own Cloudflare tunnel token. Environment-scoped, never repo-scoped, so a workflow outside `environment: beta` cannot read them. See [`../runbooks/BETA-DEPLOY.md`](../runbooks/BETA-DEPLOY.md).                            |
| `APP_DOMAIN_BETA`                                                 | repo `vars`                          | repository                                                        | `beta.afframe.com`; non-sensitive, mirrors `APP_DOMAIN_STAGING` / `APP_DOMAIN_PRODUCTION`. No `ADMIN_DOMAIN_BETA` — the beta portal has no separate admin host.                                                                                                                |
...
Two more exist for narrower purposes: `secrets-drift` (read-only SSM role, no
protection rules — a wait timer would hang the scheduled run) and `beta` (the
beta.afframe.com portal, branch policy `main-beta`, created as part of the
first-deploy gate in [`../runbooks/BETA-DEPLOY.md`](../runbooks/BETA-DEPLOY.md)).
