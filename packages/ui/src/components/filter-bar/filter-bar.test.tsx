...
function Harness() {
  const { columns, filters, actions, strategy } = useFilterBar({
...
describe("FilterBar", () => {
  ...
  it("adds a filter and shows a pill chip", async () => {
    ...
    const pill = document.querySelector('[data-slot="filter-bar-pill"]')
  ...
  it("clear-all removes all filters", async () => {
    ...
    await user.click(await screen.findByRole("option", { name: /Todo/i }))
    expect(
      document.querySelector('[data-slot="filter-bar-pill"]'),
    ).not.toBeNull()
