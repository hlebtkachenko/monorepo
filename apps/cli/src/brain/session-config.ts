...
export function buildPostingKickoff(plan: BrainPostingPlan): string {
  ...
  return [
    ...
    "2. Resolve each account NUMBER you decide to use (see step 3) to its id with a TARGETED lookup: call",
    `   mcp__afframe__list_accounts with periodId="${posting.periodId}" AND number set to the exact account`,
    '   number (e.g. "518") — it returns just that one account; read its id. Do NOT call list_accounts without a',
    "   number: a full period chart is large and overflows the tool-output limit. Prefer the 3-digit synthetic account.",
