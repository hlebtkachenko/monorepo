# Runbooks

Repeatable, ordered procedures for engineering and operating the repository.
Choose the narrowest runbook matching the task.

## Engineering workflows

- [`ADDING-X-TO-MONOREPO.md`](ADDING-X-TO-MONOREPO.md)
- [`APP-SHELL-PANELS.md`](APP-SHELL-PANELS.md)
- [`CODEGRAPH.md`](CODEGRAPH.md)
- [`COMPONENT-MIGRATION.md`](COMPONENT-MIGRATION.md)
- [`CONDUCTOR-WORKTREE-HEALTH.md`](CONDUCTOR-WORKTREE-HEALTH.md)
- [`ENDPOINT-ADDITION-RUNBOOK.md`](ENDPOINT-ADDITION-RUNBOOK.md)
- [`LOCAL-DEV.md`](LOCAL-DEV.md)
- [`SHOWCASE.md`](SHOWCASE.md)

## Deployment and infrastructure

- [`AWS-SETUP.md`](AWS-SETUP.md)
- [`CI-TURBO-REMOTE-CACHE.md`](CI-TURBO-REMOTE-CACHE.md)
- [`DB-ACCESS.md`](DB-ACCESS.md)
- [`DEPLOY.md`](DEPLOY.md)
- [`ENV-POWER.md`](ENV-POWER.md)
- [`ROLLBACK.md`](ROLLBACK.md)
- [`STAGING.md`](STAGING.md)
- [`STATUS-PAGE.md`](STATUS-PAGE.md)

## Security, secrets, and incidents

- [`AUTH-FORENSICS.md`](AUTH-FORENSICS.md)
- [`COST-INCIDENT.md`](COST-INCIDENT.md)
- [`DAST-NUCLEI.md`](DAST-NUCLEI.md)
- [`DR-DRILL.md`](DR-DRILL.md)
- [`INCIDENT.md`](INCIDENT.md)
- [`SECRETS-ADD-DELETE.md`](SECRETS-ADD-DELETE.md)
- [`SECRETS-ROTATION.md`](SECRETS-ROTATION.md)
- [`SUPPLY-CHAIN-INCIDENT.md`](SUPPLY-CHAIN-INCIDENT.md)
- [`SUPPLY-CHAIN-VERIFY-AT-DEPLOY.md`](SUPPLY-CHAIN-VERIFY-AT-DEPLOY.md)
- [`VAULT-OPS.md`](VAULT-OPS.md)

## Agent and Brain operations

- [`AGENT-HITL.md`](AGENT-HITL.md)
- [`BRAIN-OPERATOR-SESSION.md`](BRAIN-OPERATOR-SESSION.md)
- [`M2-OPERATOR-BOOTSTRAP-PROMPT.md`](M2-OPERATOR-BOOTSTRAP-PROMPT.md)
- [`mlive.example.sh`](mlive.example.sh)
