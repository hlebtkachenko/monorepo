...
- filing: DPPO input + validity layers — field parsers (@workspace/filing/fields: parseKoruna/parseSazba/parseDic/parseEpoDate + fieldTypeFor), offline IČO/DIČ mod-11 checksum (@workspace/filing/business-validity), and warn-only EPO kritické-kontroly (checkDppo, @workspace/filing/dppo-checks); demo-dppo tester now normalizes fields on blur and shows a non-blocking warnings panel
- filing: Tier 3 FÚ EPO DPPO engine (DPPDP9 v05.01.01) — generateDppo/readDppo, validateFiling(…, "dppo", "05.01.01"), buildDppoFromAccounting, computeDppoTotals (@workspace/filing/dppo-compute), vendored official XSD, plus a dev-only /[orgSlug]/demo-dppo round-trip tester
