...
The výkaz zisku a ztráty reports the rezerva na daň z příjmů (účet 599) in L.1 Daň z příjmů splatná instead of F.4 Rezervy v provozní oblasti, which § 27 vyhlášky limits to účtová skupina 55 — the VZZ no longer understates ř. 49 Výsledek hospodaření před zdaněním by the provision, and the DPPO ř. 10 derivation matches it (it also stops dropping the převodové účty 597/598 while counting their 697/698 counterparts). The obratová předvaha allocates its rounding the same way when shown v celých tisících, so its subtotals and the MD = Dal totals foot on the printed page.
