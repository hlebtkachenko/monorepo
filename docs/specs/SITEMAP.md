...
- **Debug** _(dev reference)_ — **Debug** landing · **Archetype Blank** (the Blank archetype reference page: ContentHeader without view tabs · no toolbar · one full-height Empty section · no footer) · **Archetype Details** (the Details archetype reference page: ContentHeader without view tabs · no toolbar · multiple stacked sections · a Save/Discard footer) · **Archetype Table** (the Table archetype reference page: the section-based ArchetypeTable — toolbar + faceted/multi-filter + the row Inspector rail — over mock invoice rows) · **Section Details Form** (the Form section reference page: two-column group — title + description left, a 6-column field grid right, fields spanning 1–6 columns)
