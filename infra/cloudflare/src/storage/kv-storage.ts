...
export class KvStorage implements StorageInterface {
  ...
  ): Promise<ListResultWithMetadata> {
  ...
  async readWithMetadata(
    ...
    data: ReadableStream | undefined
    metadata: Metadata | undefined
  }> {
    const value = await this.KV_STORE.getWithMetadata<KvMetadata>(key, {
      type: "stream",
    })
    return {
  ...
  private transformMetadata(
    kvMetadata: KvMetadata | null | undefined,
  ): Metadata | undefined {
