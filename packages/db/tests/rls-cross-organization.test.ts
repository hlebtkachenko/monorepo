...
describe("RLS cross-organization isolation", () => {
  ...
  describe("auth_invite isolation", () => {
    let orgAId: string
    let orgBId: string
    ...
    beforeAll(async () => {
      ...
      // Seed an invite for org A
      const [invite] = await adminSql<Array<{ id: string }>>`
        INSERT INTO auth_invite (organization_id, workspace_id, token_hash, email, role, expires_at)
        VALUES (
          ${orgAId}::uuid,
          ${workspaceId}::uuid,
          'test-token-hash-a-' || substr(md5(random()::text), 1, 8),
          'invited-a@test.invalid',
          'member',
          now() + interval '7 days'
        )
        RETURNING id
    ...
    it("org B session cannot see org A invite", async () => {
      const rows = await userSql.begin(async (tx) => {
        await tx.unsafe(
          `SELECT set_config('app.organization_id', '${orgBId}', true)`,
        )
        return tx.unsafe<Array<{ id: string }>>(
          `SELECT id FROM auth_invite WHERE id = '${inviteAId}'::uuid`,
        )
      })
      const ids = rows.filter((r) => r.id === inviteAId)
      expect(ids).toHaveLength(0)
    })

    it("org A session sees org A invite", async () => {
      const rows = await userSql.begin(async (tx) => {
        await tx.unsafe(
          `SELECT set_config('app.organization_id', '${orgAId}', true)`,
        )
