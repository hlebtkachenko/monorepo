  ...
  const r = await one<{ id: string }>(
    ...
    sql`INSERT INTO partial_record
          (organization_id, individual_record_id, quantity, measure_unit, unit_price,
           base_amount, vat_rate, vat_mode, vat_jurisdiction, supply_kind, vat_deductible, advance_settlement, vat_amount,
        ...
        VALUES
          (${ctx.organizationId}::uuid, ${individualRecordId}::uuid, ${p.quantity ?? null}, ${p.measureUnit ?? null}, ${p.unitPrice ?? null},
           ${p.baseAmount}, ${p.vatRate ?? null}, ${p.vatMode}, ${p.vatJurisdiction ?? null}, ${p.supplyKind ?? null}, ${p.vatDeductible ?? true}, ${p.advanceSettlement ?? false}, ${vatAmount},
