...
| 10  | Backup: restic → Cloudflare R2 (primary) + Backblaze B2 (secondary)                                          | Two unrelated providers; AES-256-GCM encryption                                                                                     |
...
3. **Cloudflare Access policy**:
   ...
   - Identity provider: Google Workspace + email OTP fallback
   - Policy: Allow if email = `hleb@hapd.cz`
...
- Shamir unseal keys are still available in escrow (last-resort fallback if OIDC ever breaks)

**Rollback**:

- If OIDC regresses post-revocation, use Shamir keys to seal Vault, then re-init with a fresh root token (data is preserved on the Raft volume). This is the "last resort" path documented in [`VAULT-OPS.md`](../runbooks/VAULT-OPS.md).
...
- Update `docs/runbooks/AWS-DEPLOY.md`:
...
| `docs/runbooks/AWS-DEPLOY.md`                                    | M8                            | Update references at `:177,239,241,552,553`                                                                                                                                                                                                                                                     |
