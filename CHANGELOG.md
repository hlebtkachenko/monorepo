...
- UI: Form section layout pass — container-query responsive columns (stack the title above the fields on a narrow panel instead of cramming), `px-4`/`py-4` padding (2× the panel header), h2 (`Heading level={2}`) title; new `sectionSpace` gap section + a section `fill` flag so Empty fills while Form/Space take natural height and the body scrolls
