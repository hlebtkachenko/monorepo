 ...
 * `Request` / `NextRequest`. Mirrors `apps/web/lib/request-origin.ts`.
 *
 * Behind Cloudflare Tunnel → Fargate the admin container listens on
 * `http://0.0.0.0:3100`, so `request.url` reflects the listener, not the
 * user-visible origin. A redirect built with `new URL(path, request.url)`
 * emits `Location: https://0.0.0.0:3100/...`, which the browser refuses.
 *
 * Priority:
