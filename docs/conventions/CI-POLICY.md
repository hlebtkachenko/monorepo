...
| `secrets-drift.yml` runs daily but the Vault auth block is commented out                       | Vacuously-green "monitoring"                   | M5 stub: Vault JWT auth lands in M5 (AFF-245); the daily schedule is the intended cadence once auth is wired. The skip path could emit `::warning::` but the schedule stays.                                                |
