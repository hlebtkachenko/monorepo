...
function useStoreContext(consumer: string) {
  ...
  if (!ctx) throw new Error(`\`${consumer}\` must be used within \`Banners\``)
  return ctx
}
...
function useBannerContext(consumer: string) {
  const ctx = React.useContext(BannerContext)
  if (!ctx) throw new Error(`\`${consumer}\` must be used within \`Banner\``)
  return ctx
}
...
function useBanner() {
  ...
  return React.useMemo(() => {
    const onRemove =
      id && storeCtx ? () => storeCtx.onBannerRemove(id) : undefined
    return { id, variant, dismissible, onClose, onRemove }
  }, [id, variant, dismissible, onClose, storeCtx])
}
...
function Banners({
  ...
  const banners = useStore(store, (s) => s.banners)
  const heights = useStore(store, (s) => s.heights)
...
function BannerImpl({ banner, side, index }: BannerImplProps) {
  ...
  const removing = useStore(store, (s) => s.removing.has(banner.id))
  const banners = useStore(store, (s) => s.banners)
  const heights = useStore(store, (s) => s.heights)
  ...
  if (!removing) offsetBeforeRemoveRef.current = offset
  ...
  const ctxValue = React.useMemo<BannerContextValue>(
    () => ({
      id: banner.id,
  ...
  const renderProps = React.useMemo<BannerRenderProps>(
    () => ({
      id: banner.id,
      ...(banner.variant !== undefined ? { variant: banner.variant } : {}),
      dismissible,
      onClose,
      onRemove,
    }),
...
function Banner({
  ...
  if (isControlled) openRef.current = openProp
...
function BannerIcon({ className, asChild, ...iconProps }: DivProps) {
  const Comp = asChild ? Slot.Root : "div"
  return (
    <Comp
...
function BannerContent({ className, asChild, ...contentProps }: DivProps) {
  const Comp = asChild ? Slot.Root : "div"
  return (
    <Comp
...
function BannerActions({ className, asChild, ...actionsProps }: DivProps) {
  const Comp = asChild ? Slot.Root : "div"
  return (
    <Comp
