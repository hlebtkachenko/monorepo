...
must be confirmed before or at v1. GitHub Issues own completion status,
assignees, and implementation work. This file defines stable acceptance
criteria only.
...
- Define exact v1 endpoint scope and verify generated OpenAPI, SDK, and MCP
  output against it.
- Provide a production-safe API-key issuance path.
- Decide whether sandbox and webhooks are v1 requirements or explicit
  post-launch contracts. Their concept specifications live under
  [`specs/api/`](../specs/api/).
- Ensure throttling, idempotency, request IDs, errors, and status-page coverage
  match every public claim.
- Publish only SDK, CLI, and MCP installation paths that are actually
  distributed. In-repo private packages do not count as public releases.
...
- **5. Verify OpenStatus + cloudflared health on the OVH VPS.** Use
  `infra/openstatus/deploy/keepalive.sh` when recovery is needed. Then
  apply monitor changes in the dashboard
