...
describe("publicOrigin", () => {
  ...
  it("uses x-forwarded-host + x-forwarded-proto when set", () => {
    const req = mkRequest("http://0.0.0.0:3100/", {
      "x-forwarded-host": "admin-staging.afframe.com",
      "x-forwarded-proto": "https",
    })
    expect(publicOrigin(req)).toBe("https://admin-staging.afframe.com")
  })
  ...
  it("falls back to BETTER_AUTH_URL when no forwarded host", () => {
    process.env.BETTER_AUTH_URL = "https://admin-staging.afframe.com"
    const req = mkRequest("http://0.0.0.0:3100/")
    expect(publicOrigin(req)).toBe("https://admin-staging.afframe.com")
  })

  it("falls back to request origin when no header and no env", () => {
    const req = mkRequest("http://localhost:3110/")
    expect(publicOrigin(req)).toBe("http://localhost:3110")
  })

  it("prefers x-forwarded-host over BETTER_AUTH_URL", () => {
    process.env.BETTER_AUTH_URL = "https://wrong.example.com"
    const req = mkRequest("http://0.0.0.0:3100/x", {
