...
const inputShape = {
  ...
  "seriesId": z.string().uuid().describe("INVENTORY_COUNT number series (see GET number-series)."),
  "countDate": z.string().describe("Datum inventury (§29-30)."),
  "description": z.string().max(2000).nullable().describe("Popis inventurního soupisu.").optional(),
...
export function registerCreateInventoryCount(
  server: McpServer,
  ...
  server.registerTool(
    "create_inventory_count",
    {
      ...
      annotations: {
        ...
        ...getAnnotations("createInventoryCount"),
      },
    },
    async (args): Promise<CallToolResult> => {
      try {
        const raw = args as Record<string, unknown>
        const params = { header: { "idempotency-key": raw["idempotency-key"] } } as unknown as NonNullable<operations["createInventoryCount"]["parameters"]>
        const paramKeys = new Set(["idempotency-key"])
        const bodyFields = Object.fromEntries(Object.entries(raw).filter(([k]) => !paramKeys.has(k)))
        const body = bodyFields as unknown as components["schemas"]["CreateInventoryCountRequest"]
        const init = { body, params }
        const { data, error, response } = await client.POST("/v1/accounting/inventory-counts", init)
