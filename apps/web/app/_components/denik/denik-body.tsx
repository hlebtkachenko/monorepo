...
import { JOURNAL_ROWS, JOURNAL_TABS, type JournalRow } from "./data"
 ...
 * inspector. Reads from a fixture until wired to the SDK.
 */
export function DenikBody() {
  ...
  const tabFiltered = React.useMemo(() => {
    ...
    if (!tab?.kind) return JOURNAL_ROWS
    return JOURNAL_ROWS.filter((row) => row.side === tab.kind)
  }, [activeTab])
