...
| BRAIN-CLIENT-001   | Afframe Brain local operator client (Claude Code + `@afframe/cli` + `@afframe/mcp` stdio bridge on the operator's Mac) | operator endpoint | Hleb | Medium | Customer + Financial | Tier 3 | self (operator Mac) + Anthropic | Active | Unprivileged external client of the accounting API (ADR-0025): holds no DB creds, resolves the org server-side from a user-bound agent key, and every write is server-gated + HELD. Reads client 2025 accounting documents. Pre-launch, Hleb-only. See `docs/AFFRAME-BRAIN.md`. |
...
| ANTHROPIC-001      | Anthropic Claude API — the LLM behind Afframe Brain (accounting extraction + booking proposals over client 2025 documents) via the Agent SDK | SaaS (LLM API) | Hleb | High | Customer + Financial | Tier 2 | Anthropic | Active |
