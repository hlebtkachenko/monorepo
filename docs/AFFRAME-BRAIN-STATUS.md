...
| **M2** — Supervised production: the human-review marathon | Book a real org's 2025 accounting, everything HELD, review/correct each until ≥10 clean runs, zero confident-wrong | **ENGINEERING PRs MERGED (2026-07-10); marathon still NEXT (process, not started).** The M2 pre-work train landed on `main`: booking-template library + model routing (#643, M2.1), librarian distillation engine (#644, M2.2), evidence-gate floor route-around closes (#645, M2.4), and DPH ř.12/13 §108 residual + RENT place-of-supply (#669, #540). Pre-M2 engineering already in (OCR fail-closed leg #554, issued-EU reverse-charge #541, shadow-scoring). The human-review runs themselves (W2.3–W2.5) — the load-bearing long pole — have **not** begun.                                                                                                                                                                |
| **M3** — Calibration fit + safe field-by-field lift       | Fit calibration from M2 runs; un-floor base-score fields behind server re-verification; re-gate                    | **CODE ITEMS IN REVIEW (unmerged); fit still data-gated.** Refit MATH is built + tested (`calibration.ts` + `refit.test.ts`). The M3 code items are built and open as unmerged, brain-gate-pending PRs: run-log ingestion pipeline (#646, M3.3), server-side extraction re-verifier (#647, M3.1, built-not-activated), calibration-into-gate wiring + F1 shadow preservation (#648, M3.2, default-safe). The fit itself + field-by-field lift stay data-triggered (need ≥10 shadow-scored M2 runs).                                                                        |
